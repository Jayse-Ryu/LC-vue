<template>
  <div class="main_role" id="main_role"
       :style="this.$parent.$data.mix_data.slide_mover"
       @click="$parent.$options.methods.only_close">
    <div class="p1_main">
      <section class="p1_sec_0">
        <div class="p1_sec_0_con_1">
          <p>The ability to analyze digital media is driven by</p>
          <p>countless experiences and efforts.</p>
        </div>
      </section>

      <section class="p1_sec_1"></section>

      <section class="p1_sec_2"></section>
    </div>
  </div>
</template>

<script>
  export default {
      name: "about",
  }
</script>

<style lang="scss" scoped>
  /*
  ===============================================
    Page 1
  ===============================================
  */

  .p1_main {
    position: relative;
    top: 0;
    left: 0;
    z-index: 400;
    background-image: url('../assets/unsplash1_low.jpg');
    background-size: cover;
    background-repeat: no-repeat;
    background-attachment: fixed;
    background-position: 50% 50%;
  }

  .p1_sec_0 {
    position: relative;
    top: 0;
    left: 0;
    z-index: 450;
    width: 100%;
    height: 100vh;
    padding-top: calc(50vh - 25px);
    color: yellow;
    text-align: center;
    font-size: 50px;
    font-weight: lighter;
    word-break: keep-all;
    white-space: pre-line;
    background-color: rgba(0,0,0,0.5);
    box-shadow: inset 0px -200px 20px -190px rgba(0,0,0,0.75);
  }

  .p1_sec_0_con_1 {
    position: relative;
    width: 100%;
    top: 0;
    left: 0;
  }

  @media (max-width: 1200px) {
    .p1_sec_0 {
      position: relative;
      top: 0;
      left: 0;
      z-index: 400;
      width: 100%;
      height: 100vh;
      padding-top: calc(50vh - 25px);
      color: yellow;
      text-align: center;
      font-size: 28px;
      word-break: keep-all;
      white-space: pre-line;
    }
  }/*=*/

  .p1_sec_1 {
    width: 100%;
    height: 400px;
    background-color: midnightblue;
  }

  .p1_sec_2 {
    width: 100%;
    height: 400px;
    background-color: #002a80;
  }
</style>
